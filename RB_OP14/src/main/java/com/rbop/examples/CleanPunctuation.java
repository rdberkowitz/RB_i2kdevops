...
public class CleanPunctuation extends SingleRowRule {
    ...
    public RuleResult evaluate(ResultSet rs , Map<String, String> UpdateRow) {
        Map<String, String> cols = FixedLists.Punctuation;
        List<String> keys = Arrays.asList(new String[]{"Region", "Country", "Country_Region", "Basin", "Leasing_Area", "Block", "Field", "Formation", "Well"});
        //List<String> keys = Arrays.asList(new String[]{"Region", "Country", "Country_Region", "Basin", "Leasing_Area", "Block", "Field", "Formation", "Well", "Rock_Type", "Geologic_Age", "Onshore_Offshore"});
        ...
        try {
            ...
            for (String k : keys) {
                ...
                if (!(item.equals(""))) {
                    ...
                    String item_ = item;
                    //Check if any string in the item contains a punctuation
                    for (String c : chars) {
                    ...
                    if (item_index.size() > 0) {
                        ...
                        errors.put(k, "SUGGESTION: "+item_+" (Clean punctuation before evaluating " + item_index.toString()+"); ");
                    }
                }
                ...
                if (Updater) {
                    if (!(item2.equals(""))) {
                        List<String> item_index = new ArrayList<>();
                        String item_ = item2;
                        //Check if any string in the item contains punctuation
                        for (String c : chars) {
                            //Turn map string into regex
                            String v = cols.get(c);
                            Pattern vp = Pattern.compile(v);
                            Matcher m = vp.matcher(item_);
                            //Add to count for each character
                            int count = 0;
                            while (m.find()) {
                                count++;
                            }
